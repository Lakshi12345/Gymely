...
function BillingCenter() {
    ...
    const getGeneratedBy = (bill: any) => {
        return bill.generatedBy || bill.createdBy || bill.updatedBy || "Unknown";
    };
    ...
    const filteredBillings = useMemo(() => {
        return billings.filter((bill) => {
            const billDate = normalizeDate(getBillDate(bill));

            // DATE

            if (filters.startDate && billDate) {
            ...
            if (filters.receiptType !== "All" && getReceiptType(bill) !== filters.receiptType) {
                return false;
            }

            // TAX

            if (filters.taxType !== "All" && getTaxType(bill) !== filters.taxType) {
                return false;
            }

            // SOLD BY

            if (filters.soldBy !== "All" && getSoldBy(bill) !== filters.soldBy) {
                return false;
            }

            // GENERATED BY

            if (filters.generatedBy !== "All" && getGeneratedBy(bill) !== filters.generatedBy) {
                return false;
            }
    ...
    const statistics = useMemo(() => {
        ...
        filteredBillings.forEach((bill) => {
            ...
            const method = getPaymentMethod(bill).toLowerCase().replace(/\s+/g, "");
            ...
            if (getBalance(bill) > 0) {
                pending += getBalance(bill);
            }
        });
    ...
    const generateExcelReport = () => {
        ...
        const reportData = filteredBillings.map((bill, index) => ({
            "#": index + 1,
                                    ...
                                    {loading ? (
                                        Array.from({ length: 6 }).map((_, index) => (
                                            <tr key={index} className="border-b border-gray-100">
                                                {Array.from({ length: 19 }).map((__, cellIndex) => (
                                                    <td
                                                        key={cellIndex}
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="border-r border-gray-100 px-5 py-5">
                                                        ...
                                                        {Number(bill.currentInstallment ?? 0) !==
